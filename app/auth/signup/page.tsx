...
export default function SignupPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      router.push("/onboarding"); // → onboarding instead of dashboard
    } catch (err: any) {
    ...
    <div style={{ minHeight:"100vh",background:S.bg,display:"grid",placeItems:"center",fontFamily:"Inter,sans-serif",padding:20 }}>
      ...
      <div style={{ width:"100%",maxWidth:400 }}>
        ...
        <div style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:18,padding:32 }}>
          ...
          <p style={{ fontSize:13,color:S.muted,textAlign:"center",marginBottom:28,lineHeight:1.6 }}>
            Join teams replacing Apollo.io with AI that actually works
          </p>

          {/* Benefits */}
          <div style={{ marginBottom:24 }}>
            {[
              "10 AI agents — free forever",
              "No credit card required",
              "Set up in 5 minutes",
            ].map((b,i)=>(
              <div key={i} style={{ display:"flex",alignItems:"center",gap:8,marginBottom:8 }}>
                <span style={{ color:S.accent,fontSize:14 }}>✓</span>
                <span style={{ fontSize:13,color:S.muted }}>{b}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ textAlign:"center",fontSize:12,color:S.faint }}>
            ...
            <a href="/auth/login" style={{ color:S.accent,fontWeight:600,textDecoration:"none" }}>Sign in →</a>
          </div>
        </div>

        <p style={{ textAlign:"center",fontSize:11,color:S.faint,marginTop:16,lineHeight:1.6 }}>
          By signing up, you agree to our Terms of Service.<br/>
          Apollo.io charges $49-$119/user. We start at $0.
        </p>
      </div>
    </div>
  );
