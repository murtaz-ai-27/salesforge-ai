...
export default function SequencesPage() {
  ...
  const [steps, setSteps] = useState<Step[]>([
    { id:"1",type:"email",day:1,subject:"Quick question about {{company}}",body:"Hi {{firstName}},\n\nI noticed {{company}} recently {{icebreaker}}. I wanted to reach out because SalesForge AI helps {{role}}s like yourself book 3x more meetings...\n\nWould you be open to a 15-min call?\n\nBest,\n{{sender}}",aiWritten:false },
    { id:"2",type:"linkedin",day:3,subject:"",body:"Hi {{firstName}}, sent you an email about {{topic}}. Would love to connect!",aiWritten:false },
  ...
  const generateAIStep = async () => {
    ...
    try {
      const res = await fetch("/api/ai",{ method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({ type:"emailWriter",prompt:`Write step ${activeStep.day} of a sales sequence. Type: ${activeStep.type}. Day ${activeStep.day}. Use {{firstName}} and {{company}} placeholders. Write personalized content.` }) });
      const data = await res.json();
      if (data.result) {
        setSteps(prev=>prev.map(s=>s.id===activeStep.id?{...s,body:data.result,aiWritten:true}:s));
        setActiveStep(prev=>prev?{...prev,body:data.result,aiWritten:true}:null);
        showToast("AI step written ✓");
      }
    } catch {}
    ...
    <div style={{ background:S.bg,minHeight:"100vh",fontFamily:"Inter,sans-serif" }}>
      ...
      <div style={{ marginLeft:240,padding:"28px 32px" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:28 }}>
          {SEQUENCES.map(seq=>(
            <div key={seq.id} style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:16,padding:22 }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14 }}>
                {[["Open",`${seq.openRate}%`,S.accent],["Reply",`${seq.replyRate}%`,"#818cf8"],["Meeting",`${seq.meetingRate}%`,"#34d399"]].map(([l,v,c])=>(
              ...
              <div style={{ fontSize:12,color:S.faint }}><span style={{ color:S.text,fontWeight:600 }}>{seq.prospects}</span> prospects · {seq.steps} steps · {seq.channel}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:16,overflow:"hidden" }}>
          <div style={{ padding:"16px 20px",borderBottom:`1px solid ${S.lineSoft}` }}>
              ...
              {steps.map((step)=>{
                ...
                return (
                  <div key={step.id} onClick={()=>setActiveStep(step)}
                    style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,marginBottom:6,cursor:"pointer",border:`1px solid ${activeStep?.id===step.id?"rgba(200,255,0,0.2)":S.lineSoft}`,background:activeStep?.id===step.id?"rgba(200,255,0,0.05)":"rgba(255,255,255,0.02)" }}>
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      ...
                      <div style={{ fontSize:11,color:S.faint,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{step.subject||step.body.slice(0,28)+"..."}</div>
                    </div>
                    {step.aiWritten&&<span style={{ fontSize:9,fontWeight:700,color:S.accent,background:"rgba(200,255,0,0.1)",padding:"2px 6px",borderRadius:999 }}>AI</span>}
                  </div>
                );
              })}
                ...
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
                  {STEP_TYPES.map(st=>(
                    <button key={st.type} onClick={()=>addStep(st.type)}
                      style={{ display:"flex",alignItems:"center",gap:6,padding:"8px 10px",borderRadius:9,background:"rgba(255,255,255,0.03)",border:`1px solid ${S.lineSoft}`,color:S.muted,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"Inter,sans-serif" }}>
            ...
            {activeStep ? (
              <div style={{ padding:24 }}>
                <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20 }}>
                  <div>
                    <div style={{ fontSize:15,fontWeight:700,color:S.text }}>Day {activeStep.day} — {STEP_TYPES.find(t=>t.type===activeStep.type)?.label}</div>
                    <div style={{ fontSize:12,color:S.faint,marginTop:2 }}>Use {"{{firstName}}"}, {"{{company}}"} as variables</div>
                  </div>
                ...
                <textarea value={activeStep.body}
                  ...
                  style={{ width:"100%",minHeight:220,padding:"14px",borderRadius:10,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",color:S.text,fontSize:13,fontFamily:"Inter,sans-serif",outline:"none",resize:"vertical",lineHeight:1.7 }}
              ...
              <div style={{ display:"grid",placeItems:"center",color:S.faint,fontSize:14 }}>Select a step to edit</div>
            )}
          </div>
        </div>
      </div>
