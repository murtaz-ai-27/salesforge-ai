...
const PIPELINE = [
  { stage:"New Prospects",count:1284,value:0,color:"#555a66" },
  { stage:"Contacted",count:847,value:0,color:"#818cf8" },
  { stage:"Replied",count:263,value:284000,color:S.accent },
  { stage:"Meeting Booked",count:47,value:188000,color:"#34d399" },
  { stage:"Proposal Sent",count:18,value:126000,color:"#f59e0b" },
  { stage:"Closed Won",count:7,value:84000,color:"#C8FF00" },
];
...
function MiniBar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <div style={{ display:"flex",alignItems:"center",gap:10 }}>
      ...
      <span style={{ fontSize:11,color:S.faint,minWidth:32,textAlign:"right" }}>{pct}%</span>
    </div>
  );
}
...
export default function AnalyticsPage() {
  ...
  const totalRevenue = PIPELINE.reduce((a,s)=>a+s.value,0);
    ...
    <div style={{ background:S.bg,minHeight:"100vh",fontFamily:"Inter,sans-serif" }}>
      ...
      <div style={{ marginLeft:240,padding:"28px 32px" }}>
          ...
          <div style={{ display:"flex",gap:6 }}>
            {["7d","30d","90d","All"].map(p=>(
              <button key={p} onClick={()=>setPeriod(p)}
                style={{ padding:"7px 14px",borderRadius:9,border:`1px solid ${period===p?"rgba(200,255,0,0.3)":S.lineSoft}`,background:period===p?"rgba(200,255,0,0.08)":"transparent",color:period===p?S.accent:S.faint,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"Inter,sans-serif",transition:"all 0.2s" }}>
                {p}
              </button>
            ))}
          </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24 }}>
          {[
            { label:"Total Emails Sent",value:"3,817",delta:"+24%",sub:"vs last period",color:S.accent },
            { label:"Total Replies",value:"1,184",delta:"+31%",sub:"31% reply rate",color:"#818cf8" },
            { label:"Meetings Booked",value:"47",delta:"+18%",sub:"via AI SDR",color:"#34d399" },
            { label:"Pipeline Value",value:"$284K",delta:"+42%",sub:"active deals",color:"#f59e0b" },
          ].map(k=>(
            <div key={k.label} style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:14,padding:"18px 20px" }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                <span style={{ fontSize:12,color:"#34d399",fontWeight:700 }}>▲ {k.delta}</span>
                <span style={{ fontSize:11,color:S.faint }}>{k.sub}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20 }}>
              ...
              {WEEKLY.map(d=>(
                <div key={d.day} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,height:"100%" }}>
                  <div style={{ flex:1,width:"100%",display:"flex",flexDirection:"column",justifyContent:"flex-end",gap:3 }}>
                    <div style={{ width:"100%",borderRadius:"4px 4px 0 0",background:`rgba(200,255,0,0.15)`,height:`${(d.replies/maxEmails)*100}%`,minHeight:4,transition:"height 0.5s",position:"relative" }}>
                      <div style={{ position:"absolute",inset:0,background:"linear-gradient(180deg,#818cf8,rgba(129,140,248,0.3))",borderRadius:"4px 4px 0 0" }}/>
                    </div>
                    <div style={{ width:"100%",borderRadius:"4px 4px 0 0",background:"linear-gradient(180deg,#C8FF00,rgba(200,255,0,0.2))",height:`${(d.emails/maxEmails)*100}%`,minHeight:8,transition:"height 0.5s" }}/>
                  </div>
          ...
          <div style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:16,padding:22 }}>
              ...
              {WEEKLY.map(d=>(
                <div key={d.day} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,height:"100%" }}>
                  <div style={{ flex:1,width:"100%",display:"flex",flexDirection:"column",justifyContent:"flex-end" }}>
                    <div style={{ position:"relative",width:"100%",borderRadius:"4px 4px 0 0",background:"linear-gradient(180deg,#34d399,rgba(52,211,153,0.15))",height:`${(d.meetings/maxMeetings)*100}%`,minHeight:8,transition:"height 0.5s" }}>
                      {d.meetings===maxMeetings && (
                        <div style={{ position:"absolute",top:-20,left:"50%",transform:"translateX(-50%)",fontSize:10,fontWeight:700,color:"#34d399",whiteSpace:"nowrap" }}>Peak</div>
                      )}
                    </div>
                  </div>
                  <div style={{ fontSize:10,color:S.faint,marginTop:6 }}>{d.day}</div>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:6 }}>
              ...
              <span style={{ fontSize:11,color:S.faint }}>Meetings Booked · Weekly Total: <span style={{ color:"#34d399",fontWeight:700 }}>{WEEKLY.reduce((a,d)=>a+d.meetings,0)}</span></span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20 }}>
          ...
          <div style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:16,padding:22 }}>
            ...
            <div style={{ fontSize:12,color:S.faint,marginBottom:20 }}>Total pipeline: <span style={{ color:"#f59e0b",fontWeight:700 }}>${(totalRevenue/1000).toFixed(0)}K</span></div>
              ...
              <div key={stage.stage} style={{ marginBottom:14 }}>
                <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                    ...
                    <span style={{ fontSize:13,color:S.text,fontWeight:500 }}>{stage.stage}</span>
                  </div>
                  <div style={{ display:"flex",alignItems:"center",gap:12 }}>
                    <span style={{ fontSize:13,fontWeight:700,color:stage.color }}>{stage.count}</span>
                    {stage.value>0 && <span style={{ fontSize:11,color:S.faint }}>${(stage.value/1000).toFixed(0)}K</span>}
                  </div>
                </div>
                <MiniBar value={stage.count} max={PIPELINE[0].count} color={stage.color}/>
              </div>
            ))}
          </div>
          ...
          {/* Top Performers */}
          <div style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:16,padding:22 }}>
            <div style={{ fontSize:14,fontWeight:700,color:S.text,marginBottom:4 }}>Top Performers</div>
            <div style={{ fontSize:12,color:S.faint,marginBottom:20 }}>AI Agent vs your best reps</div>
            {TOP_REPS.map((rep,i)=>(
              <div key={rep.name} style={{ display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:i<TOP_REPS.length-1?`1px solid ${S.lineSoft}`:"none" }}>
                ...
                <div style={{ flex:1,minWidth:0 }}>
                  ...
                  <div style={{ fontSize:11,color:S.faint,marginTop:2 }}>{rep.emails} emails · {rep.rate} reply rate</div>
                </div>
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:14,fontWeight:700,color:i===0?S.accent:S.text,fontFamily:"Syne,sans-serif" }}>{rep.meetings}</div>
        ...
        {/* Email Performance */}
        <div style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:16,padding:22 }}>
          <div style={{ fontSize:14,fontWeight:700,color:S.text,marginBottom:4 }}>Email Performance by Channel</div>
          <div style={{ fontSize:12,color:S.faint,marginBottom:20 }}>Comparing deliverability, opens, replies, and meetings across channels</div>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14 }}>
            {[
              { channel:"Email", sent:2840, delivered:"94%", opened:"67%", replied:"31%", meetings:38, color:S.accent },
              { channel:"LinkedIn", sent:640, delivered:"99%", opened:"82%", replied:"18%", meetings:7, color:"#60a5fa" },
              { channel:"Phone", sent:180, delivered:"71%", opened:"—", replied:"23%", meetings:2, color:"#34d399" },
              { channel:"SMS", sent:157, delivered:"97%", opened:"89%", replied:"12%", meetings:0, color:"#f59e0b" },
            ].map(c=>(
              <div key={c.channel} style={{ background:"rgba(255,255,255,0.02)",border:`1px solid ${S.lineSoft}`,borderRadius:12,padding:16 }}>
                <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:14 }}>
                  <div style={{ width:8,height:8,borderRadius:2,background:c.color }}/>
                  <span style={{ fontSize:13,fontWeight:700,color:S.text }}>{c.channel}</span>
                </div>
                {[["Sent",c.sent.toLocaleString()],["Delivered",c.delivered],["Opened",c.opened],["Replied",c.replied],["Meetings",String(c.meetings)]].map(([l,v])=>(
                  <div key={l} style={{ display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${S.lineSoft}` }}>
                    <span style={{ fontSize:11,color:S.faint }}>{l}</span>
                    <span style={{ fontSize:12,fontWeight:600,color:l==="Replied"||l==="Meetings"?c.color:S.text }}>{v}</span>
                  </div>
                ))}
              </div>
            ))}
