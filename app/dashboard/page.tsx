...
export default function DashboardPage() {
  ...
  // Fetch prospects when user is ready
  useEffect(() => {
    if (!user?.uid) return;
  ...
  const kpis = [
    { label:"Prospects Today", value:(stats?.prospectsToday ?? prospects.length).toLocaleString(), delta:"+23% vs yesterday", color:S.accent },
    ...
    { label:"Active Sequences", value:String(stats?.activeSequences ?? 3), delta:`${stats?.prospectsInSequences ?? 847} enrolled`, color:S.accent },
          ...
          <div style={{ background:S.panel,border:`1px solid ${S.lineSoft}`,borderRadius:16,overflow:"hidden" }}>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px",borderBottom:`1px solid ${S.lineSoft}` }}>
              <div>
                ...
                <div style={{ fontSize:12,color:S.faint,marginTop:2 }}>
                  {prospects.length > 0 ? `${prospects.length} prospects from Supabase ✓` : "Loading from Supabase..."}
                </div>
              </div>
            ...
            {prospects.length === 0 ? (
              <div style={{ padding:"32px 24px",textAlign:"center" }}>
                <div style={{ fontSize:13,color:S.faint }}>Loading prospects from database...</div>
              </div>
            ) : prospects.slice(0,5).map((p,i) => (
              <div key={p.id} style={{ display:"grid",gridTemplateColumns:"1fr 110px 70px 90px 100px",padding:"13px 20px",borderBottom:i<4?`1px solid ${S.lineSoft}`:"none",cursor:"pointer",transition:"background 0.2s" }}
